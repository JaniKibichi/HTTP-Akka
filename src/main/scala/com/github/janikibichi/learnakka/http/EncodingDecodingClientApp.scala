...
object EncodingDecodingClientApp extends App {
  implicit val actorSystem = ActorSystem()
  implicit val materializer = ActorMaterializer()

  import actorSystem.dispatcher
  val http = Http()
  val uriServer = "http://127.0.0.1:8088"
  ...
  private def compress(input: String, encoder: Encoder): ByteString = encoder.encode(ByteString(input))
}