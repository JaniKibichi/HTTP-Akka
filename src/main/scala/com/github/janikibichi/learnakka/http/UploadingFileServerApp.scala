      ...
      path("regularupload"){
        entity(as[Multipart.FormData]) { formData =>
          val partsFuture = formData.parts.mapAsync(1){
            b => b.toStrict(2.seconds).map(_.entity.data.utf8String)
          }.runFold(List.empty[String])(_ :+ _)

          onSuccess(partsFuture) { allParts =>
            complete {
              val wholeFile = allParts.mkString
              s"Regular upload: submitted file has ${wholeFile.split("n").size } lines"
            }
          }
        }
      }~path("streamupload"){
        entity(as[Multipart.FormData]) { formData =>
          ...
          { b =>
            b.entity.dataBytes
              .via(Framing.delimiter(ByteString("n"),1024,allowTruncation=true))
              .map(_ => 1)
              .runReduce(_+_)
          }.runReduce(_+_)

          onSuccess(linesFuture) { lines =>
            complete {
...
object UploadingFileServerApp extends App{
